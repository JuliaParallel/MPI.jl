...
* **`INTRODUCED`** — the MPI standard version that added each procedure. The
  ...
  MPI library in use raises a `FeatureLevelError` naming the version it needs
...
Run it by hand; the output is committed. It is not part of `gen.sh` because it
reaches out to the network and its result only changes when a new MPI standard
version is released.
...
Get the two include directories from any Julia with the matching `MPICH_jll`
installed, e.g.
...
* **`pympistandard` wheels from PyPI.** Each release ships one `apis.json`
  ...
  authoritative. Releases 0.1.0/0.1.1 carry the MPI-4.0 dataset (515 procedures),
  0.1.2/0.2.0 the MPI-5.0 one (567). `apis.json` also gives every parameter's
  *kind*; a `POLY*` kind is precisely the standard's marker for a parameter that
  widens in the large-count interface, which is where `EMBIGGENED` comes from —
  more reliable than grepping generated code for a `_c` suffix.
* **MPICH headers** for the two boundaries `apis.json` cannot resolve, since only
  two datasets are published: MPICH 3.4.3 is MPI 3.1, MPICH 4.3.2 is MPI 4.1.
...
and is commented out. The MPI standard's own change log
...
would carry no information, and the oldest MPICH available as a JLL is 3.3.2 —
there is no public machine-readable way to split 1.0/2.0/2.1/2.2/3.0/3.1 apart.
...
procedures MPI.jl has always version-gated by hand, because it does support
libraries older than those. `test/test_neighbor_*.jl` asserts
...
whose per-version tags (`mpi-4.0`, `mpi-4.1`, `mpi-5.0`) carry the authoritative
...
For a member, the diff is reproducible with
