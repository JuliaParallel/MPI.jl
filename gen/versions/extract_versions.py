...
Run by hand, not from ``gen.sh``; the output is committed.  See README.md.
...
* ``pympistandard`` wheels from PyPI.  Each ships one ``apis.json`` extracted from
  ...
  releases 0.1.0/0.1.1 carry the MPI-4.0 dataset, 0.1.2/0.2.0 the MPI-5.0 one.
...
* MPICH headers, to resolve the boundaries ``apis.json`` cannot: MPICH 3.4.3 is
  MPI 3.1, MPICH 4.3.2 is MPI 4.1.
...
def main():
    ...
    # functions (MPI_CONVERSION_FN_NULL) also carry POLY parameters, but they are
    # typedefs and constants rather than callable entry points -- MPI defines no
    # `MPI_User_function_c()` to call -- so they must not get a wrapper.
    embiggened = sorted(
        e["name"]
    ...
    with out.open("w") as f:
        ...
        f.write("# MPI 3.0 or later, so a version would carry no information.\n")
