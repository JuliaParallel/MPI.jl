const MPIDatatype = [Char,
                     Int8, UInt8, Int16, UInt16, Int32, UInt32, Int64,
                     UInt64,
                     Float32, Float64, Complex64, Complex128]
@compat const MPIBuffertype{T} = Union{Ptr{T}, Array{T}, Ref{T}}
...
type Status
    val::Array{Cint,1}
    Status() = new(Vector{Cint}(MPI_STATUS_SIZE))
end
...
function Waitall!(reqs::Array{Request,1})
    ...
    statvals = Matrix{Cint}(MPI_STATUS_SIZE, count)
    ccall(MPI_WAITALL, Void,
          (Ptr{Cint}, Ptr{Cint}, Ptr{Cint}, Ptr{Cint}),
          &count, reqvals, statvals, &0)
    stats = Vector{Status}(count)
    for i in 1:count
...
function Waitsome!(reqs::Array{Request,1})
    ...
    indices = Vector{Int}(outcount)
    stats = Vector{Status}(outcount)
    for i in 1:outcount
        ind = Int(inds[i])
...
function Reduce{T}(sendbuf::MPIBuffertype{T}, count::Integer,
    ...
    isroot = Comm_rank(comm) == root
    recvbuf = Vector{T}(isroot ? count : 0)
...
function Scatterv{T}(sendbuf::MPIBuffertype{T},
                     ...
                     comm::Comm)
    recvbuf = Vector{T}(counts[Comm_rank(comm) + 1])
...
function Gather{T}(sendbuf::MPIBuffertype{T}, count::Integer,
                   root::Integer, comm::Comm)
    isroot = Comm_rank(comm) == root
    recvbuf = Vector{T}(isroot ? Comm_size(comm) * count : 0)
...
function Allgather{T}(sendbuf::MPIBuffertype{T}, count::Integer,
                      comm::Comm)
    recvbuf = Vector{T}(Comm_size(comm) * count)
...
function Gatherv{T}(sendbuf::MPIBuffertype{T}, counts::Vector{Cint},
    ...
    recvbuf = Vector{T}(isroot ? sum(counts) : 0)
